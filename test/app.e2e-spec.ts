...
describe('Test main line game Hide-Santa', () => {
	...
	it('registration participants', async () => {
		...
		const res_end_3 = await request(app.getHttpServer())
			.get('/game/api/my-recipient/'+res_id_3+'?'+queryString.stringify({gameId: 1}, {arrayFormat: 'bracket'}))
			.set('Accept', 'application/json')
			.expect('Content-Type', /json/)
			.expect(200)
