...
describe('Test not recipient Hide-Santa', () => {
	...
	it('registration participants in two games', async () => {
		...
		const res_end_2 = await request(app.getHttpServer())
			.get('/game/api/my-recipient/'+1)
			.set('Accept', 'application/json')
			.expect('Content-Type', /json/)
			.expect(409)

		expect(res_end_2.body.reason).toEqual('Unable to find recipient "1" in game "2"');

		const res_end_3 = await request(app.getHttpServer())
			.get('/game/api/my-recipient/'+2+'?'+queryString.stringify({gameId: 4}, {arrayFormat: 'bracket'}))
			.set('Accept', 'application/json')
			.expect('Content-Type', /json/)
			.expect(409)

		expect(res_end_3.body.reason).toEqual('Unable to find recipient "2" in game "4"');
	});
});
